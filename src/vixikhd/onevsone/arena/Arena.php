...
class Arena implements Listener
{
		...
		for ($lS = 1; $lS <= $this->data["slots"]; $lS++) {
			if (!$selected) {
				if (!isset($this->players[$index = "spawn-$lS"])) {
					$player->teleport(Position::fromObject(Vector3::fromString($this->data["spawns"][$index]), $this->world));
	...
	public function startRestart(): void {
		...
		$ev->call();
		$this->plugin->getServer()->broadcastMessage("§a[1vs1] Player {$player->getName()} won the match at {$this->world->getFolderName()}!");
		$this->phase = self::PHASE_RESTART;
	}
	...
	public function onMove(PlayerMoveEvent $event): void {
		...
		if ($this->inGame($player)) {
			...
			if ($event->getPlayer()->getLocation()->distance(Vector3::fromString($this->data["spawns"][$index])) > 1) {
	...
	public function loadArena(bool $restart = false): void {
		...
		if (!$this->world instanceof World) $this->world = $this->plugin->getServer()->getWorldManager()->getWorldByName($this->data["level"]);
